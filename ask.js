...
(function () {
    ...
    let config = { model: 'glm-5.3-flash', models: ['glm-5.3-flash'], effort: 'max', hasKey: true, localAgent: 'auto', agentExec: true, intentModel: 'glm-5.3-flash' };
    ...
    function buildHtmlPreviewSource(source) {
        ...
        const clean = raw.replace(/<meta[^>]+http-equiv\s*=\s*["']?content-security-policy["']?[^>]*>/gi, '');
    ...
    schedule();
        ...
        if (/<\/head\s*>/i.test(clean))
            return clean.replace(/<\/head\s*>/i, runtime + '</head>');
        if (/<head[^>]*>/i.test(clean))
            return clean.replace(/<head[^>]*>/i, match => match + runtime);
        if (/<html[^>]*>/i.test(clean))
            return clean.replace(/<html[^>]*>/i, match => match + '<head>' + runtime + '</head>');
        if (/<!doctype\s+html\s*>/i.test(clean))
            return clean.replace(/<!doctype\s+html\s*>/i, match => match + runtime);
    ...
    function enhanceCode(container, autoRender = false) {
        container.querySelectorAll('pre').forEach(pre => {
            ...
            if (isHtml)
                wrap.classList.add('html-capable');
            ...
            if (isHtml) {
                ...
                htmlRenderBtn.onclick = () => {
                    ...
                    const syncHeight = event => {
                        if (event.source !== frame.contentWindow || !event.data?.__glmHtmlPreview)
                            return;
    ...
    function renderMsg(m, idx) {
        ...
        if (m.role === 'user') {
            ...
            if (m.quote)
                bubble.appendChild(renderQuoteBox(m.quote));
            ...
            if (m.reasoning)
                content.appendChild(buildReasoning(m));
    ...
    function buildReasoning(m) {
        const box = document.createElement('div');
        if (m.reasoningCollapsed === undefined)
    ...
    function latestReasoningLine(reasoning) {
    ...
    function buildToolTrace(m) {
        ...
        root.addEventListener('toggle', () => {
            if (root.open)
                toolTraceGroupOpen.add(groupKey);
            else
                toolTraceGroupOpen.delete(groupKey);
        });
        ...
        for (const t of m.tools) {
            ...
            row.addEventListener('toggle', () => {
                if (row.open)
                    toolTraceOpen.add(key);
                else
                    toolTraceOpen.delete(key);
            });
    ...
    function upsertToolTrace(m, tool) {
        ...
        if (idx >= 0)
            m.tools[idx] = { ...m.tools[idx], ...tool, startedAt: m.tools[idx].startedAt || tool.startedAt };
        else
            m.tools.push(tool);
        refreshMsgEl(m);
    }
    ...
    async function respond(delegated = false, existing = null) {
        ...
        if (session === owner) {
            if (!existing)
                els.thread.appendChild(renderMsg(m, owner.messages.length - 1));
            else
                refreshMsgEl(m);
            els.scroll.scrollTop = els.scroll.scrollHeight;
        }
        ...
        const paint = (force = false) => {
            if (session !== owner || !owner.messages.includes(m))
                return;
            const now = Date.now();
            ...
            onEvent(ev) {
                ...
                if (ev.type === 'done') {
                    ...
                    if (ev.ok && ev.aborted)
                        m.aborted = true;
                    if (!ev.ok)
                        m.error = ev.error || '请求失败';
                    if (owner.streamingReqId === reqId)
        ...
        try {
            ...
            if (!summary)
                summary = await window.askAPI.localAgents();
    ...
    async function appendEditImages(fileList) {
        ...
        if (!imageFiles.length || !editingData || !editingRender)
            return false;
        ...
        const renderEditAttachments = () => {
            ...
            addImg.onclick = async () => {
                ...
                if (r?.ok && r.images?.length) {
                    if (data.images.length + r.images.length > 6)
                        return toast('最多添加 6 张图片');
    ...
    async function addImages(urls) {
        if (!urls.length)
            return;
        if (pending.images.length + urls.length > 6)
            return toast('最多添加 6 张图片');
    ...
    els.imagePreview.addEventListener('click', e => {
        if (e.target === els.imagePreview || e.target.closest('#preview-close'))
            closeImagePreview();
    });
    ...
    function agentLabelText(summary = agentSummary) {
        if (config.localAgent === 'direct')
            return '仅GLM';
        if (config.localAgent !== 'auto')
            return AGENTS.find(a => a.id === config.localAgent)?.label || 'Agent';
    ...
    function toggleMenu(menu, show) {
        ...
        if (willShow) {
            if (menu === els.modelMenu)
                renderModelMenu();
            if (menu === els.effortMenu)
                renderEffortMenu();
            if (menu === els.agentMenu)
                renderAgentMenu();
    ...
    els.moreMenu.addEventListener('click', e => {
        ...
        if (row.dataset.act === 'history')
            openHistory();
        else if (row.dataset.act === 'settings')
            window.askAPI.openSettings();
        else if (row.dataset.act === 'quit')
            window.askAPI.quit();
    });
    els.plusMenu.addEventListener('click', async (e) => {
        ...
        if (r && r.ok) {
            if (r.images)
                await addImages(r.images);
            if (r.files)
                await addFiles(r.files);
        }
        else if (r && r.error) {
            toast(r.error);
        }
    ...
    els.attachChips.addEventListener('click', e => {
        ...
        if (item?.querySelector('img'))
            openImagePreview(item.querySelector('img').src);
    });
    ...
    setInterval(() => {
        ...
        if (root) {
            ...
            if (title)
                title.textContent = `执行中 · ${elapsed}s`;
            root.querySelectorAll('.tool-item.running').forEach(item => {
                ...
                if (state && startedAt)
                    state.textContent = `运行 ${Math.max(1, Math.round((Date.now() - startedAt) / 1000))}s`;
            });
        }
    }, 1000);
})();
